(() => {
  class ChronoTabParser {
    ...
    _findAndGroupChronos() {
      ...
      this._tabSheet.staffs.forEach((staff, staffIndex) => {
        for (let t = 0; t < staff[0].length; t++) {
          let chrono = this.constructor.getChronoAt(t, staff);
          if (!chrono) { continue; }
          let adjacentChrono = this.constructor.getChronoAt(t+1, staff);
          if (adjacentChrono) {
            let merged = this.constructor.mergeChronos(chrono, adjacentChrono);
            if (merged) {
              chrono = merged;
            } else {
              adjacentChrono = null;
    ...
    static mergeChronos(chronoA, chronoB) {
      let a = chronoA.split(this.chronoNoteSeparator);
      let b = chronoB.split(this.chronoNoteSeparator);
      ...
      return merged.join(this.chronoNoteSeparator);
    }
    ...
    static getChronoAt(index, staff) {
      ...
      return chrono.split('').join(this.chronoNoteSeparator);
    }
  ...
  document.extutil.addProperty('chronos', 'ChronoTabParser', ChronoTabParser, false);
})();
